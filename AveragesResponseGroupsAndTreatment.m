...
legend('Control mouse 2','Antibody dose','location','northwest')
ylim([0 8000])
...
set(gca,'FontSize',20)
set(gca,'FontName','Times New Roman')
legend('Control mouse 1*','Control mouse 1','Control mouse 2','Control mouse 3','Antibody dose','location','northwest')
ylim([0 8000])
...
RCalldays = [RCday0,RCday2,RCday3,RCday7,RCday12,RCday17,RCday20];

RClabels = [zeros(size(RCday0)),2*ones(size(RCday2)),3*ones(size(RCday3)),...
          7*ones(size(RCday7)),12*ones(size(RCday12)),...
          17*ones(size(RCday17)),20*ones(size(RCday20))];

figure
boxplot(RCalldays,RClabels,'Whisker',5,'colors','r')
xlabel('Day')
ylabel('Tumor volume (mm$^3$)')
title('Slow growth, control')
set(gca,'FontSize',20)
set(gca,'FontName','Times New Roman')
...
NCday3 = [RC1(2,3),NC1(2,3),NC2(2,3),NC3(2,3)];
% Nday5 = [NC2(2,4),NC3(2,4)];
NCday7 = [RC1(2,4),NC1(2,4),NC2(2,5),NC3(2,5)];
NCday12 = [RC1(2,5),NC1(2,5),NC2(2,6),NC3(2,6)];
NCday17 = [RC1(2,6),NC1(2,6)];
NCday20 = [RC1(2,7),NC1(2,7)];
...
RCmaxes = [max(RCday0),max(RCday2),max(RCday3),...
           max(RCday7),max(RCday12),max(RCday17),max(RCday20)];
RCmins = [min(RCday0),min(RCday2),min(RCday3),...
          min(RCday7),min(RCday12),min(RCday17),min(RCday20)];
RCaverages = [mean(RCday0),mean(RCday2),mean(RCday3),...
             mean(RCday7),mean(RCday12),mean(RCday17),mean(RCday20)];
RCmedians  = [median(RCday0),median(RCday2),median(RCday3),...
            median(RCday7),median(RCday12),median(RCday17),median(RCday20)];
RCstdevs   = [std(RCday0),std(RCday2),std(RCday3),std(RCday7),...
            std(RCday12),std(RCday17),std(RCday20)];
RCsizes = [numel(RCday0),numel(RCday2),numel(RCday3),...
          numel(RCday7),numel(RCday12),numel(RCday17),numel(RCday20)];
...
RCquartiles(end+1,:) = quantile(RCday3,[0.25, 0.75]);
RCquartiles(end+1,:) = quantile(RCday7,[0.25, 0.75]);
RCquartiles(end+1,:) = quantile(RCday12,[0.25, 0.75]);
RCquartiles(end+1,:) = quantile(RCday17,[0.25, 0.75]);
RCquartiles(end+1,:) = quantile(RCday20,[0.25, 0.75]);

figure
% errorbar(allexpdaysbut5,RCmedians,RCmedians'-RCquartiles(:,1),RCquartiles(:,2)-RCmedians','LineWidth',3)
errorbar(allexpdaysbut5,RCaverages,RCaverages-RCmins,RCmaxes-RCaverages,'LineWidth',3)
hold
plot(allexpdaysbut5,RCmedians,'ro','MarkerFaceColor','r','MarkerSize',10)
xlabel('Day')
ylabel('Tumor volume (mm$^3$)')
title('Slow growth, control')
set(gca,'FontSize',20)
...
%%
 save('ExpoGroupTumorVolumesV2.mat')
